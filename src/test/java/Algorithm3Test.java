...
import org.cifasis.mc1.poet.org.cifasis.mc1.Algorithm3;
...
/**
 * Created by Cristian on 6/10/15.
 */

@FixMethodOrder(MethodSorters.JVM)
public class Algorithm3Test {

    private Algorithm3 explore(EventStructure es, int m, int n) {
        System.out.println(es.toString());
        System.out.println("m=" + m + " n=" + n + " lfs-bound=" + Algorithm2.computeBound(m, n));
        Algorithm3 algorithm = new Algorithm3(es, m, n);
        Set<EventStructure.Event> C = Sets.newTreeSet();
        C.add(es.getRoot());
        algorithm.explore(C, new TreeSet<EventStructure.Event>(), new TreeSet<EventStructure.Event>(), new TreeSet<EventStructure.Event>());
    ...
    private Algorithm3 explore(EventStructure es, int lfsBound) {
        System.out.println(es.toString());
        System.out.println("Manual lfs-bound=" + lfsBound);
        Algorithm3 algorithm = new Algorithm3(es, lfsBound);
        Set<EventStructure.Event> C = Sets.newTreeSet();
        C.add(es.getRoot());
        algorithm.explore(C, new TreeSet<EventStructure.Event>(), new TreeSet<EventStructure.Event>(), new TreeSet<EventStructure.Event>());
        System.out.println("ES events count: " + es.getEventSet().size());
